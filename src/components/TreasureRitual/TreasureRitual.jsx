...
export default function TreasureRitual() {
  ...
  useEffect(() => {
    ...
    const initGallery = () => {
      if (galleryAppRef.current) return;
      const anchor = document.getElementById('gallery-anchor');
      if (!anchor) return;

      galleryAppRef.current = new App(anchor, {
        items: artifacts,
        bend: 2.2,
        textColor: '#CBB57A',
        borderRadius: 0.08,
    ...
    const handleChestClick = () => {
      if (chest.classList.contains('open-chest')) {
        ...
        if (galleryAppRef.current && galleryAppRef.current.hideCards) {
          galleryAppRef.current.hideCards();
        }
    ...
    const handleChestOpen = () => {
      ...
      revealTimeout = window.setTimeout(() => {
        ...
        // Trigger card reveal animation after gallery is ready
        window.setTimeout(() => {
          if (galleryAppRef.current && galleryAppRef.current.revealCards) {
            galleryAppRef.current.revealCards();
          }
        }, 500);
      }, 1200);
    };
